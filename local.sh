...
EOF
    ...
    if $COMPOSE; then
        ...
        export SCOPES=$SCOPES && export BUILD_URL=$BUILD_URL && export LOADTEST_HOST=$LOADTEST_HOST  && export DB_TABLENAME=$DB_TABLENAME && docker-compose up $ABORT_ON_EXIT)
        ...
        docker run -i --network="bridge" -v $PWD/reports:/opt/reports \
        -v $PWD/credentials:/meta/credentials -p 8090:8089 -e ROLE=standalone -e TARGET_HOST=$TARGET \
        ...
        -e SEND_ANONYMOUS_USAGE_INFO=$KPI -e BUILD_URL=$BUILD_URL -e AWS_ACCESS_KEY_ID=$AWS_ACCESS_KEY_ID -e AWS_SECRET_ACCESS_KEY=$AWS_SECRET_ACCESS_KEY -e LOADTEST_HOST=$LOADTEST_HOST -e DB_TABLENAME=$DB_TABLENAME $IMAGE
    fi
}
